...
class SpeechToText {
        ...
        try {
            ...
            processorNode.port.onmessage = (event: MessageEvent) => {
                ...
                if (audioBuffer.length >= BUFFER_SIZE) {
                    const wavData: Blob = SpeechToText.convertToWavFormat(audioBuffer);
                    socket.send(wavData);
    ...
    private static convertToWavFormat(samples: number[]): Blob {
        const buffer: ArrayBuffer = new ArrayBuffer(44 + samples.length * 2);
        const view: DataView = new DataView(buffer);

        // Schreiben des WAV-Header
        SpeechToText.writeString(view, 0, 'RIFF'); // ChunkID
        view.setUint32(4, 32 + samples.length * 2, true); // ChunkSize
        SpeechToText.writeString(view, 8, 'WAVE'); // Format
        SpeechToText.writeString(view, 12, 'fmt '); // Subchunk1ID
        ...
        SpeechToText.writeString(view, 36, 'data'); // Subchunk2ID
        view.setUint32(40, samples.length * 2, true); // Subchunk2Size

        // Schreiben der Samples
        SpeechToText.floatTo16BitPCM(view, 44, samples);
    ...
    private static floatTo16BitPCM(view: DataView, offset: number, input: number[]): void {
